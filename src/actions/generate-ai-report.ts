...
type GenerateAIReportSchema = z.infer<typeof generateAIReportSchema>;
...
async function generateAIReport({ month }: GenerateAIReportSchema) {
  ...
  if (!hasPremiumPlan)
    throw new Error("User is not premium to generate AI reports");

  if (!process.env.OPEN_AI_API_KEY) return DUMMY_REPORT;
  ...
  ${transactions.map((transaction) => `${transaction.date.toLocaleString("pt-BR")}-R$${transaction.amount}-${transaction.type}-${transaction.category}`).join(";")}
  `;
